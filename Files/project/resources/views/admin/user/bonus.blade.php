    ...
    <div class="card-body">
      <form class="geniusform" action="{{ route('admin.gs.update') }}" method="POST" enctype="multipart/form-data">
          ...
          {{ csrf_field() }}
            ...
            <div class="form-group">
                <label for="affilate_new_user">{{ __('Referral New User Bonus') }} ({{ $curr->name }})</label>
                <input type="number" class="form-control" id="affilate_new_user"  placeholder="{{ __('Referral New User Bonus') }}" name="affilate_new_user" value="{{ showBladeAdminAmount($gs->affilate_new_user) }}" required="" min="0" step="0.1">
            </div>

            <div class="form-group">
                <label for="affilate_user">{{ __('Referral User Bonus') }} ({{ $curr->name }})</label>
                <input type="number" class="form-control" id="affilate_user"  placeholder="{{ __('Referral User Bonus') }}" name="affilate_user" value="{{ showBladeAdminAmount($gs->affilate_user) }}" required="" min="0" step="0.1">
            </div>
